...
echo -e "${GREEN}🔧 Applying Grafana fix with --network host...${NC}"
...
# Apply fix via SSH
echo -e "${YELLOW}🔧 Applying Grafana fix...${NC}"
...
echo "Starting new Grafana container with --network host..."
docker run -d \
  --name grafana \
...
echo "Grafana fix applied successfully!"
EOF

echo -e "${GREEN}✅ Grafana fix applied!${NC}"
echo -e "${GREEN}🌐 Access Grafana at: http://$GRAFANA_IP:3000${NC}"
echo -e "${YELLOW}💡 Credentials: admin / admin123${NC}"
echo -e "${YELLOW}💡 Using --network host for AWS IMDS access${NC}"
