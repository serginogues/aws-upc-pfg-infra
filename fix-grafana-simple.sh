...
echo -e "${GREEN}🔧 Fixing Grafana with simple AWS access...${NC}"
...
# Fix AWS access via SSH
echo -e "${YELLOW}🔧 Configuring Grafana with AWS access...${NC}"
...
echo "Starting new Grafana container with AWS access..."
docker run -d \
  --name grafana \
...
echo "Grafana AWS access fix completed!"
EOF

echo -e "${GREEN}✅ Grafana AWS access fixed!${NC}"
echo -e "${GREEN}🌐 Access Grafana at: http://$GRAFANA_IP:3000${NC}"
echo -e "${YELLOW}💡 Credentials: admin / admin123${NC}"
echo -e "${YELLOW}💡 Using --network host for AWS access${NC}"
