...
echo -e "${GREEN}🔧 Fixing Grafana AWS authentication...${NC}"
...
# Fix AWS authentication via SSH
echo -e "${YELLOW}🔧 Updating Grafana container with AWS credentials...${NC}"
...
echo "Grafana AWS authentication fix completed!"
EOF

echo -e "${GREEN}✅ Grafana AWS authentication fixed!${NC}"
echo -e "${GREEN}🌐 Access Grafana at: http://$GRAFANA_IP:3000${NC}"
echo -e "${YELLOW}💡 Credentials: admin / admin123${NC}"
echo -e "${YELLOW}💡 The CloudWatch datasource should now work with the correct region${NC}"
